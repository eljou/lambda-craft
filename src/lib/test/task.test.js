...
describe.skip('Task type class', () => {
  ...
  describe('toPromise', () => {
    it('should transform to a resolved Promise', () => pipe(Task.of('good'), Task.toPromise).then(s => assert.equal(s, 'good')))
    it('should transform to a rejected Promise', () => pipe(Task.rejected('bad'), Task.toPromise).catch(err => assert.equal(err, 'bad')))
  })
})
